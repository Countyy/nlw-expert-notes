...
export function NoteCard({ note, onNoteDeleted }: NoteCardProps) {
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md bg-slate-800 p-5 overflow-hidden relative hover:ring-2 hover:ring-slate-600 transition text-left focus-visible:ring-2 focus-visible:ring-lime-400 outline-none flex flex-col gap-3">
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="overflow-hidden inset-0 md:inset-auto fixed md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full bg-slate-700 md:rounded-md flex flex-col outline-none md:h-[60vh]">
          ...
          <button
            type="button"
            className="bg-slate-800 py-4 text-sm text-slate-300 outline-none text-center font-medium group"
